...
(function (global) {
  ...
  function bootScroll() {
    ...
    if (typeof Lenis !== 'undefined' && !reduce && !global.__lenis) {
      lenis = new Lenis({ lerp: 0.1, smoothWheel: true });
      global.__lenis = lenis;                 // 히어로 라이브러리(heroes.js)가 재사용 → 이중 부팅 방지
  ...
  /* blur — 포커스&블러 진입(00_scroll 2-9). 모바일은 opacity만(성능) */
  register('blur', function (el, ctx) {
    var m = global.matchMedia && global.matchMedia('(max-width:820px)').matches;
    gsap.fromTo(el, m ? { opacity: 0 } : { filter: 'blur(14px)', opacity: 0.25 },
      { filter: 'blur(0px)', opacity: 1, duration: 1, ease: 'power2.out',
        scrollTrigger: { trigger: el, start: 'top 82%', once: true } });
  });

  /* skew — 스크롤 속도 왜곡(잔상)(00_scroll 3-15). 멈추면 0 복귀. tier로 강약 */
  register('skew', function (el, ctx) {
    var set = gsap.quickSetter(el, 'skewY', 'deg');
    var clamp = gsap.utils.clamp(-10, 10);
    var proxy = { s: 0 };
    ScrollTrigger.create({
      onUpdate: function (self) {
        var sk = clamp(self.getVelocity() / -340) * ctx.pFactor;
        if (Math.abs(sk) > Math.abs(proxy.s)) {
          proxy.s = sk;
          gsap.to(proxy, { s: 0, duration: 0.6, ease: 'power3', overwrite: true, onUpdate: function () { set(proxy.s); } });
        }
      }
    });
  });

  /* words — 글자(단어 단위) 등장: 텍스트를 단어로 쪼개 마스크 아래에서 떠오름(00_scroll 2-8).
     <br>·<em> 등 자식 태그는 보존. 모든 섹션 제목 진입 애니의 표준. */
  register('words', function (el, ctx) {
    if (!el.dataset.fxWords) {
      el.dataset.fxWords = '1';
      ...
      [].forEach.call(el.childNodes, function (node) {
        if (node.nodeType === 3) {
          node.textContent.split(/(\s+)/).forEach(function (tok) {
            ...
            var mask = document.createElement('span'); mask.className = 'fx-w';
    ...
    gsap.from(el.querySelectorAll('.fx-w > span'), {
      yPercent: 115, opacity: 0, duration: 0.85, ease: 'power3.out',
      stagger: 0.055, delay: parseFloat(el.getAttribute('data-delay')) || 0,
      scrollTrigger: { trigger: el, start: 'top 86%', once: true }
    });
  });
